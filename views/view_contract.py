...
def view_contracts(contracts, clients):
    """
    Affiche les détails des contrats avec les clients associés.
    """
    if contracts:
        table_data = []
...
def view_wich_contract(contracts, client):
    """
    Affiche les contrats associés à un client et permet à l'utilisateur de sélectionner un contrat.
    """
    if contracts:
        ...
        selected_idx = click.prompt(
            "Sélectionner le numéro du contrat ", type=int)
...
def view_update_contract(contract, client_name, modified):
    """
    Affiche les détails du contrat et permet à l'utilisateur de choisir ce qu'il souhaite modifier.
    """
    if modified:
        ...
        click.echo(
            f"Que souhaitez-vous modifier dans le contrat de {client_name}")
        ...
        if choice == 1:
            new_total_amount = click.prompt(
                "Entrez le nouveau montant total", type=str)
            ...
            new_remaining_amount = click.prompt(
                "Entrez un nouveau reste à charge: ", type=str)
            ...
            new_status = click.prompt(
                'Le contrat est-il signé? (O/N)', type=str)
            while new_status.upper() != 'O' and new_status.upper() != 'N':
                click.echo(
                    'Veuillez répondre par "O" pour Oui et "N" pour Non')
                new_status = click.prompt(
                    'Le contrat est-il signé? (O/N)', type=str)
            ...
            if new_client:
                click.echo(f'Le client {client_name} va être remplacé par le client {
                           new_client.full_name} pour le contrat n° {contract.id}')
...
def view_delete_contract(contract, client, deleted, events):
    """
    Affiche les détails du contrat à supprimer et demande confirmation à l'utilisateur.
    """
    if deleted:
