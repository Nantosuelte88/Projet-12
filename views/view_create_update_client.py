...
def view_create_client():
    ...
    response = click.prompt('Voulez-vous entrer un nom d\'entreprise? (O/N)', type=str)
    if response.upper() == 'O':
        ...
        if company_id is not None:
            ...
            new_client.extend([full_name, email, phone_number, company_name, company_id])
        else:
            click.echo("Le client sera créé sans entreprise associée gkdfjgkvjfdc")
            new_client.extend([full_name, email, phone_number, company_name, None])
    else:
...
def wich_customer():
    ...
    if client_name.isalnum():
        clients_corresponding = search_corresponding_client(client_name)
        
        if clients_corresponding:
            if len(clients_corresponding) == 1:
                return clients_corresponding[0]  # Retourne le seul client trouvé
            else:
                ...
                selected_idx = click.prompt("Entrez le numéro du client: ", type=int)
...
def view_update_client(client, modified):
    if modified:
        ...
        response = click.prompt(f"Souhaitez-vous modifier {client.full_name} ? (O/N) ")
        ...
        if response.upper() == "O":
            ...
            click.echo(f"4: Son entreprise : {client.company or 'Actuellement vide'}") 
            ...
            if choice == 1:
                ...
                new_phone_number = click.prompt("Entrez le nouveau numéro de téléphone: ")
                client_data = {"phone_number": new_phone_number}

            elif choice == 4:
                response = click.prompt('Voulez-vous entrer un nom d\'entreprise? (O/N)', type=str)
                if response.upper() == 'O':
                    new_company_name = click.prompt("Entrez le nom de la nouvelle entreprise: ")
                    while not all(c.isalnum() or c.isspace() for c in new_company_name):
                        click.echo("Le nom de l'entreprise ne doit contenir que des chiffres, des lettres et des espaces.")
                        new_company_name = click.prompt('Nom de l\'entreprise', type=str)
                    new_company_name, new_company_id = wich_company(new_company_name)
                    if new_company_id is not None:
                        click.echo("Le client sera associé à la nouvelle entreprise")
                        client_data = {"company_name": new_company_name, "company_id": new_company_id}
                    else:
                        click.echo("Le client sera créé sans entreprise associée")
                        client_data = {"company_name": new_company_name, "company_id": None}
                else:
                    click.echo("Le client sera créé sans entreprise associée")
